# Full Battle — End-to-End Combat Simulation
#
# This feature validates the complete battle pipeline end-to-end:
#   character creation → gear equipping → turnmeter-driven initiative →
#   attack resolution → HP tracking → battle log generation.
#
...
#
#   ✔ The battle ends before the tick ceiling (both combatants can deal
...
#   ✔ The battle log is non-empty and contains every expected event type
#
...
Feature: Full Battle — End-to-End Combat Simulation
    As a game designer
    I want to run a complete geared battle between two combatants
    So that I can verify the battle loop ends correctly and produces a full battle log
    ...
    # ──────────────────────────────────────────────────────────────────────────
    Scenario: Geared Fighter battles a Geared Orc to the death
        # Fighter setup
        ...
        # Run the battle with a generous tick ceiling.
        # Given the stats above, average damage per hit is 6–8, so the battle
        # should resolve in far fewer than 500 ticks.
        When the battle is simulated with a maximum of 500 ticks

        # ── Structural assertions ──────────────────────────────────────────────
        # The battle MUST end before the tick ceiling (not a stalemate).
        ...
        And the battle log should contain turnmeter gain events
        And the battle log should contain at least one attack event
        And the battle log should contain at least one damage event
