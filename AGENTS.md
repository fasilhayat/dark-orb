...
> **Canonical source.** This file is read by OpenCode automatically.
> `.github/copilot-instructions.md` is a copy kept in sync via `make sync-instructions` (from `src/`).
> Edit only this file, then run that command.
